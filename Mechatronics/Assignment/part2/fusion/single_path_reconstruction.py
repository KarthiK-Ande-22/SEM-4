...
def run_ekf(file_path, color, label):
    ...
    # --- Extract first heading from CSV for rotation ---
    initial_heading_deg = df["heading"].iloc[0]  # First heading value from CSV
    print(f"Initial heading (degrees): {initial_heading_deg}")
    ...
    plt.plot(x_rotated, y_rotated, label=f"Rotated {label} (Turned {initial_heading_deg:.2f}°)", 
             color=color, linewidth=2)

# Plot the rotated second trajectory
plt.figure(figsize=(10, 8))
run_ekf("path_1_telemetry.csv", "red", "2")  # Rotated plot for path 2

# Adjust axes and labels
plt.xlabel("X Position (m)")
plt.ylabel("Y Position (m)")
plt.title("Rotated Fused Sensor Trajectory for Path 1 using EKF")
plt.legend()
plt.grid()
plt.savefig("rotated_fused_trajectory_path1.png")
plt.show()
